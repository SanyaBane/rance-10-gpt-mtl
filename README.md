...
That puts `Rance10.ain`, `Rance10EX.ex` and `Rance10Pact.afa` into `build/release`, and one more `Rance10.ain` under `optional/enemy-panel/`. The three at the top are the translation and nothing else: they change what the game says and never how it plays. The one under `optional/` is the same build with the every-round enemy panel in it — the last thing described in this file — and you turn that on by copying it over the other `Rance10.ain`. Copy the files into your game folder yourself when you want to install them. The images are not in there — see two paragraphs down, they have no build command at all.
